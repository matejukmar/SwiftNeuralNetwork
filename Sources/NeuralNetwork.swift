...
public final class NeuralNetwork {
	...
	public let euler: Double = Double(M_E)
	...
	public var incomingWeights : [[[Double]]] = []  //'incoming' indicates the way arrays are structured. Each node of (next) layer has count(nodes from previpus layer) incoming waights
	public var biasWeights : [[Double]] = []

	
	/**
	Initializes empty Neural Network of any size.
	
	@param layers Array of integers. First element in array is number of input nodes. Last element in array is number of output nodes. In-between integers are number of nodes in each hidden layer.
	
	@param bias Bias value
	
	@param eta Eta value
	
	@param verbose If true network will print out debug data
	
	*/
	
	public init(layers: [Int], bias: Double, eta: Double, verbose: Bool = false) {
	...
	/**
	Initializes Neural Network with data from previously saved file.
	
	*/

	public init(dataUrl: URL, verbose: Bool = false) {
		...
		let etaData = data.subdata(in: start..<(start+MemoryLayout<Double>.size))
		let eta: Double = etaData.withUnsafeBytes { (ptr: UnsafePointer<Double>) -> Double in
			return ptr.pointee
		}
		self.eta = eta
		...
		let biasData = data.subdata(in: start..<(start+MemoryLayout<Double>.size))
		let bias: Double = biasData.withUnsafeBytes { (ptr: UnsafePointer<Double>) -> Double in
			return ptr.pointee
		}
		self.bias = bias
		...
		for layerIndex in 1..<layers.count {
			...
			for _ in 0..<layers[layerIndex] {
				...
				for _ in 0..<layers[layerIndex-1] {
					...
					let weightData = data.subdata(in: start..<(start+MemoryLayout<Double>.size))
					let weight: Double = weightData.withUnsafeBytes { (ptr: UnsafePointer<Double>) -> Double in
						return ptr.pointee
					}
		...
		var biasWeights: [[Double]] = []
		for layerIndex in 1..<layers.count-1 {
			var layerBiasWeights: [Double] = []
			for _ in 0..<layers[layerIndex] {
				let weightData = data.subdata(in: start..<(start+MemoryLayout<Double>.size))
				let weight: Double = weightData.withUnsafeBytes { (ptr: UnsafePointer<Double>) -> Double in
					return ptr.pointee
	...
	func initializeData() {
		...
		for i in 1..<layers.count { //starts with 1 because input nodes do not have incoming weights
			...
			for _ in 0..<layers[i] {
				...
				for _ in 0..<layers[i-1] {
					nodeWeights.append(Double.random(min: -1, max: 1))
					numWeights += 1
				}
				layerWeights.append(nodeWeights)
			}
		...
		for i in 1..<layers.count-1 {  //input and output nodes do not have bias weights
			var biasLayerWeights: [Double] = []
			for _ in 0..<layers[i] {
				biasLayerWeights.append(Double.random(min: -1, max: 1))
				numBiasWeights += 1
			}
	...
	func DoubleArrayString(array: [Double]) -> String {
		var result = "["
		var first = true
		for elm in array {
			if !first {
	...
	*/
	...
	@param inout Array of double values. Length of this array needs to be same as number of input nodes.
	
	*/
	
	public func eval(input: [Double]) {
	...
	func forwardPropagate(input: [Double]) {
		...
		if verbose {
			print("forward prop: \(DoubleArrayString(array: nodes[layers.count-1]))")
			print("forward propagation time \(endDate.timeIntervalSince(startDate)) seconds")
		}
	}
	
	func backwardPropagate(expectedOutput: [Double]) {
		...
		var downstreamDeltas: [Double] = []
		var currentDeltas: [Double] = []
	...
	/**
	Teaches Neural Network with input and expected output.
	
	@param input Array of double values. Length of this array needs to be same as number of input nodes.
	
	@param expectedOutput Array of double values. Length of this array needs to be same as number of output nodes.
	
	*/

	public func teach(input: [Double], expectedOutput: [Double]) {
		guard input.count == layers[0] else {
			print("incorrect length of input array")
			return
		}
		...
		if verbose {
			...
			for i in 0..<layers[layers.count-1] {
				totalError += pow(expectedOutput[i]-nodes[layers.count-1][i], 2)/2
			}
	...
	/**
	String representation of output nodes.

	*/

	public var outputNodesString: String {
		return DoubleArrayString(array: nodes[layers.count-1])
	}
